...
import {View,Text, FlatList, ActivityIndicator, Dimensions, ScrollView, RefreshControl} from 'react-native'
...
export default function GetLettersReceive(x){
...

                {
                    letters.length == 0 && (
                        <View style={{flex:1, marginTop:height/2.75, alignItems:"center"}} >
                            ...
                            <Text style={{color:"grey",fontSize:13,fontStyle:"italic"}} >No Letters Received</Text>
                        </View>
                    )
                }
    ...
    async function getLetters(){
        const token = JWT.encode(authx.currentUser.uid, "secretKey");
                const data = await (await fetch(ip+":8012/getReceivedLetter", {
                    method: 'POST',
                    headers: {
